...
fun SuccessAnimation(modifier: Modifier = Modifier) {
    ...
    Canvas(modifier.graphicsLayer { this.alpha = alpha.value }) {
        ...
        if (ringProgress.value < 1f) {
            ...
            drawCircle(
                color = circleColor.copy(alpha = (1f - ringProgress.value) * 0.5f),
                radius = ringRadius,
                center = center,
            )
        }

        // The themed disc popping in.
        drawCircle(color = circleColor, radius = INNER_RADIUS * unit * circleScale.value, center = center)
